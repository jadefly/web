        ...
        >
          ...
          <el-form-item :label="$t('Dict.AmountLabel')" prop="qty">
            <el-input
              ...
              v-model="method.qty" v-judge v-autotest>
            </el-input>
          </el-form-item>
        </el-form>
...
export default {
  ...
  methods: {
    ...
    submitChangeMethodQty() {
      if (this.repeatMark) return false;
      this.repeatMark = true;
      this.$refs.changeMethodQtyForm.validate((valid) => {
        if (valid) {
          const submitQty = this.method.qty;
          // 如果存在限定的数量则判断提示下
          if (this.method.maxQty || this.method.maxQty === 0) {
            if (submitQty > this.method.maxQty) {
              this.repeatMark = false;
