         ...
         <div class="itemListTitle">
            ...
            <div class="priceFormula">
              ...
              <p>
                ...
                <span class="sign">+</span>
                <!-- 如果是联动那个品项明细的加价随着主套餐的数量增加而 -->
                <!-- <span v-if="!pkgMainData.isNotFollowAmount">
                  {{multi(pkgNormalTotalRaisePrice.totalRaisePrice, pkgMainData.lastQty)}}
                </span> -->
                <span class="w_price" v-if="pkgMainData.raisePriceType == 1">
                ...
                <!-- <span v-if="!pkgMainData.isNotFollowAmount">
                  {{pkgNormalTotalRaisePrice.pkgListTotalMakeFee}}*{{pkgMainData.lastQty}}
                </span> -->
                <span class="w_method">{{!pkgMainData.isNotFollowAmount ? (pkgNormalTotalRaisePrice.pkgListTotalMakeFee*100)*(pkgMainData.lastQty*100)/10000 : pkgNormalTotalRaisePrice.pkgListTotalMakeFee}}</span>
                <span class="sign">=</span>
         ...
         <div class="itemListBox">
          <div class="itemListContent" :id="'itemListContent'+type" ref="addItemContent">
            <div v-for="pkgClass in pkgClasses" :key="pkgClass.pkgClassId">
              ...
              <ul id="itemListUl" v-autotest>
                <li v-for="(item, index) in (pkgClass.activePkgDtList || [])"
                  :key="item.realItemId+index" @click="itemClickHandle(item, index, pkgClass)"
                  :class="{active:activeItemId == (item.itemId + '-' + index)}"
                >
                  <p class="itemCode">{{item.itemCode || item.code}}</p>
                  <div class="item">
                    <span class="itemName">{{item.itemName || item.name}}{{ item.auxiliaryUnitId ? `(${(item.auxiliaryUnitQty || 1) + item.auxiliaryUnitName})` : ''}}</span>
                    <span class="itemMethodPrice" v-show="!isShowRaiseMemberPrice(item)">{{ item.raisePrice ? '+' + handleRaisePrice(item.raisePrice) : ''}}</span>
                    <span class="itemMethodPrice" v-show="isShowRaiseMemberPrice(item)">
                      {{ item.raiseMemberPrice || item.raiseMemberPrice == 0 ? '+' + handleRaisePrice(item.raiseMemberPrice) : ''}}
                    </span>
                    <!-- 国际化：会 -->
                  ...
                  <!-- 国际化：0_即起 -->
                  <p class="itemRemarks" v-if="(item.methodList && item.methodList.length) || (item.serveWayId && item.serveWayId != $t('Biz.Order.EqualVal'))">
                    <!-- 国际化：注: -->
                    <span>{{$t('Dict.Notes01')}}</span>
                    <span v-if="item.serveWay && item.serveWay.name">{{item.serveWay.name}}<strong v-if="item.methodList && item.methodList.length">|</strong></span>
                    <span v-if="item.serveWay && item.scId">{{item.servewayName}}<strong v-if="item.methodList && item.methodList.length">|</strong></span>
          ...
          <div class="mueContent">
            ...
            <div class="menuList">
              <ul :id="type + 'MenuList'" v-autotest>
                <li class="menuItem" v-for="(menu, index) in pkgUnSelectedItemsPager"
                  :key="menu.itemId" @click="addMenu(menu, index)"
            ...
            <div class="menuControl">
              ...
              <div class="menuSearch" :class="!!scId ? 'showCheckBoxSy':''">
                <label for="">
                  ...
                  <!-- 国际化：快速查找 -->
                  <input type="text" :id="type + 'Search'" :placeholder="$t('Dict.QuickSearch')"
                  ref="menusSearch"
                  v-switch-keyboard:top="keyboardParams" v-model.trim="menuSearch" v-judge v-autotest>
                </label>
              </div>
...
export default {
  components: {
    Dialog, Pager, PackageChangeNum, NumPager, PackageItemMethod, PackageItemServeWay,
  },
  mixins: [DialogMixin],
  props: {
    type: {
  ...
  data() {
    return {
      // 模拟下contorlBtn数据
      // contorlBtnData:[
      //   {btnName: "删除", method:"pkgDelete", disabled: false},
      //   {btnName: "+", method:"pkgAdd", disabled: false},
      //   {btnName: "-", method:"pkgSubtract", disabled: false},
      //   {btnName: "半份", method:"pkgItemHalf", disabled: false},
      //   {btnName: "数量", method:"pkgChangeNumber", disabled: false},
      //   {btnName: "做法", method:"pkgChoseMethod", disabled: false},
      //   {btnName: "上菜方式", method:"pkgServeWay", disabled: false}
      // ],
      // 联动 回退
      contorlBtnData: [
        { btnName: this.$t('Dict.Btn.DeleteItem'), method: 'pkgDelete', disabled: false },
        {
          btnName: '+', method: 'pkgAdd', disabled: false, name: 'changeNum',
        },
        {
          btnName: '-', method: 'pkgSubtract', disabled: false, name: 'changeNum',
        },
        {
          btnName: this.$t('Dict.Btn.ChangeHalfNum'), method: 'pkgItemHalf', disabled: false, name: 'changeNum',
        },
        {
          btnName: this.$t('Dict.Btn.ChangeItemNum'), method: 'pkgChangeNumber', disabled: false, name: 'changeNum',
        },
        { btnName: this.$t('Dict.Btn.ChangeItemMethods'), method: 'pkgChoseMethod', disabled: false },
        { btnName: this.$t('Dict.Btn.ChangeItemServeWay'), method: 'pkgServeWay', disabled: false },
      ],
      ...
      // 用来区分是否是已提交套餐的修改过来的
      scId: null,
      isChangeQtyType: 0, // 在联动的情况下判断主套餐的数量变化是否变更明细的数量标识
  ...
  computed: {
    ...
    // 联动回退
    contorlBtnLists() {
      // 根据判断弹框如果是通过加单完的套餐修改过来的并且是联动的时候要禁用掉去套餐明细的数量修改按钮
      const cloneDeepContorBtnData = cloneDeep(this.contorlBtnData);
      this.scId && !this.pkgMainData.isNotFollowAmount && function () {
        cloneDeepContorBtnData.forEach((btn) => {
          if (btn.name === 'changeNum' && btn.method !== 'pkgChangeNumber') {
            this.$set(btn, 'disabled', true);
          }
        });
      }.bind(this)();
      return cloneDeepContorBtnData;
    },
    hotKeySet() {
      return {
    ...
    pkgUnSelectedItems() {
      ...
      //       raisePrice: item.raisePrice, // 普通加价金额
      //       raiseMemberPrice: item.raiseMemberPrice, // 会员加价金额
    ...
    classTotal() {
      ...
      nowPkgClassData[0].activePkgDtList.forEach((list) => {
        // 如为称重确认的品项,则数量按1来计算（如为联动取套餐的数量）
        let { count } = list;
        if (list.isConfirmWeigh) count = this.scId && !this.pkgMainData.isNotFollowAmount ? this.pkgMainData.lastQty : 1;
      ...
      return {
        classData: nowPkgClassData[0],
        isOrderRemainderNum: (nowPkgClassData[0].choiceQty * 1000 - pkgClassCountTotal * 1000) / 1000,
        choiceQty: nowPkgClassData[0].choiceQty,
      };
    },
    // 套餐明细品项加价和做法加价总和 是个对象的逻辑
    pkgNormalTotalRaisePrice() {
      if (this.pkgClasses) {
        let totalRaisePrice = 0; // 套餐明细加价的总额
        ...
        this.pkgClasses.forEach((pkgCls) => {
          pkgCls.normalDtList.forEach((item) => {
            ...
            // 会员加价的总计算
            totalRaiseMemberPrice = this.countTotalRaisePrice(item, this.isShowRaiseMemberPrice(item) ? item.raiseMemberPrice : (item.raisePrice ? item.raisePrice : 0), totalRaiseMemberPrice);
            ...
            if (item.methodList && item.methodList.length) {
              ...
              item.methodList.forEach((method) => {
                if (method.price) {
                  ...
                  // 提交完的已入库套餐修改的时候明细的数量是乘以了套餐主品项数量的
                  const itemCount = this.scId && !this.pkgMainData.isNotFollowAmount
                    ? (item.count * 1000) / (this.pkgMainData.lastQty * 1000)
                    : item.count;
                  // 做法是联动的和计算
                  if (method.isRelaCount) {
                    itemMethodTotal = add(itemMethodTotal, multi(methodTotalPrice, itemCount));
                  } else {
        ...
        // totalRaisePrice 品项加价的总和 pkgListTotalMakeFee 做法价格的总额 additionalTotalPrice 以上的总额
        return {
          totalRaisePrice,
  ...
  methods: {
    ...
    countTotalRaisePrice(item, raisePrice, totalRaisePrice) {
      if (item.isRaiseByQty) {
        ...
        // 联动 回退
        // 如果是联动的明细数量的话 对于已入库的品项加价的算法要除去套餐的明细的数量
        if (this.scId && !this.pkgMainData.isNotFollowAmount) {
          totalRaisePrice = add(totalRaisePrice, raisePrice * (item.count * 1000) / (this.pkgMainData.lastQty * 1000));
        } else {
    ...
    handleChange(newVal, oldVal) {
      ...
      if (this.scId && !this.pkgMainData.isNotFollowAmount && oldVal && newVal) {
        this.pkgClasses.forEach((pkgClass, index) => {
          // 按照比列重新设置下分类套餐的限定数值返位
          this.$set(pkgClass, 'choiceMinQty', multi(pkgClass.oldChoiceMinQty, newVal));
          this.$set(pkgClass, 'choiceQty', multi(pkgClass.oldChoiceQty, newVal));
          this.$set(pkgClass, 'pkgClassName', pkgClass.pkgClassName.split('(')[0] + this.$t('Biz.Order.LongMsg016', [pkgClass.choiceMinQty, pkgClass.choiceQty]));
          pkgClass.activePkgDtList.map((list, index) => {
            // 算出变化的后数量值
            const lastCount = multi(list.proportionNum, newVal);
            const changeCount = sub(lastCount, list.count);
            const [mergeList] = pkgClass.mergePkgDtList.filter(item => list.itemId == item.itemId);
            list.count = lastCount;
            mergeList.count = changeCount < 0 ? sub(mergeList.count, Math.abs(changeCount)) : add(mergeList.count, Math.abs(changeCount));
            // 对应改变右侧的cout 数显示
            this.menuChangeCount(changeCount < 0 ? '-' : '+', Math.abs(changeCount), list);
            // 变化了就要重新赋值
            this.assignmentHandle(list, index, this.classTotal.classData);
            // console.log(mergeList)
          });
        });
      }
    },
    pkgMainPriceFn(pkgMainData) {
      if (this.scId) {
        return (pkgMainData.price * 1000 - pkgMainData.raisePrice * 1000) / 1000;
      }
      return pkgMainData.price;
    },
    /**
     * 初始化下当前请求的数据
     * 对过来的数据先进行count 属性添加 整理完return
     * raisePriceType 套餐明细的加价类型 (1.销售加价默认 2.会员加价)
     * pkgClassesNew 注这个字段本身就不该有的，，不知道谁开发自定义排序
     * 强制给这个字段，造成以前的逻辑都是按pkgClasses没new的key来写的
     * ，这样造成修改回显混乱，，，目前大致补救好但是估计以后扩展东西肯定很难的
     */
    initResData(resData) {
      ...
      if (cloneResData) {
        ...
        this.posOrderBill.bsData.memberPriceFlg ? cloneResData.package.raisePriceType = 2 : cloneResData.package.raisePriceType = 1;
        // 套餐右侧菜品品项 添加属性count 数量的key
        cloneResData.pkgUnSelectedItems.forEach((pkgSelItem, index) => {
          // 添加属性 key count
          pkgSelItem = Object.assign({}, pkgSelItem, { count: 0 });
          cloneResData.pkgUnSelectedItems.splice(index, 1, pkgSelItem);
        });
        cloneResData.pkgClasses.forEach((pkcls) => {
          // console.log(this.scId)
          // this.scId &&
          // 已下的这些判断是主要真对已提交的常规品项的套餐修改
          if (this.scId && !cloneResData.package.isNotFollowAmount) {
            this.$set(pkcls, 'oldChoiceMinQty', pkcls.choiceMinQty);
            this.$set(pkcls, 'choiceMinQty', pkcls.choiceMinQty * resData.package.lastQty);
            this.$set(pkcls, 'oldChoiceQty', pkcls.choiceQty);
            this.$set(pkcls, 'choiceQty', pkcls.choiceQty * resData.package.lastQty);
            this.$set(pkcls, 'pkgClassName', pkcls.pkgClassName.split('(')[0] + this.$t('Biz.Order.LongMsg016', [pkcls.choiceMinQty, pkcls.choiceQty]));
          }
          // console.log(pkcls)
          // 每个组
    ...
    onOpen() {
      ...
      this.pkgNormalData = null;
      this.pkgNormalData = this.initResData(this.resData);
      this.$log.info(`初始化常规套餐处理过以后的数据：${this.pkgNormalData}`);
      // 根据areaType类型的不同  当前客位所在的不同区域显示不同的价格判断 添加条件 排除下 促销的价和时价的情况
      if (this.pkgNormalData.package.discFlg != 4 && !this.pkgNormalData.package.isItemTimePrice && this.areaType != 0 && this.pkgNormalData.package.priLocals
              && (this.pkgNormalData.package.priLocals[0][`other_price${this.areaType}`] || this.pkgNormalData.package.priLocals[0][`other_price${this.areaType}`] == 0)) {
    ...
    assignmentHandle(item, index, pkgClass) {
      ...
      this.$log.info(`常规套餐账单变化重新赋值：${this.activeItemData}`);
    },
    // 找到当前正操作的品项数据
    findActivceItemData() {
      let activeIndex = 0;
    ...
    actionBtnClick(btnMethodName) {
      ...
      if (this.activeItemId && this.activeItemId != '') {
        // 做法
      ...
      this.$refs.PackageItemServeWay.open({ itemData, isPkgModule: true }, {
        submit: (serveWayData) => {
          ...
          if (serveWayData.isOrderingBillAllServeWay) {
            ...
            this.$set(findItem, 'serveWayId', serveWayData.serveWay.id);
          }
     ...
     * @标题 -添加做法
     * @函数说明 对选中的套餐明细添加做法
     * @param itemData object      代表当前所操作品项的数据
     * @param methodsData object   代表是请求过来的做法数据
     * @param pkgItem number
     * 代表是请求过来的做法数据 如果是不存在代码的是非套餐的品项, 1- 代表的是常规套餐明细的做法修改  2-代表的是宴会套餐明细的做法修改
     * @params methodData object 里的key methodList-> 代表的是所选中的做法数据 publicIsValid -> 代表的是是否对此分类的所有品项有效
     */
    pkgChoseMethod() {
      const itemData = this.activeItemData;
    ...
    pkgChangeNumber() {
      ...
      const itemChangeData = {
        num: (this.scId && !this.pkgMainData.isNotFollowAmount)
          ? div(this.activeItemData.count, this.pkgMainData.lastQty)
          : this.activeItemData.count, // 当前选择的品项数量值
        ...
        total: this.activeItemData.count, // 总数
        auxiliaryUnitQty: this.activeItemData.auxiliaryUnitId
          ? (this.scId && !this.pkgMainData.isNotFollowAmount)
            ? div(this.activeItemData.auxiliaryUnitQty || 1, this.pkgMainData.lastQty)
            : this.activeItemData.auxiliaryUnitQty || 1
          : null, // 辅助单位数量
        auxiliaryUnitName: this.activeItemData.auxiliaryUnitName || '',
      };
      // 改量弹框
      this.$refs.PackageChangeNum.open(itemChangeData, {
        submit: (changeNumData) => {
          ...
          if (changeNumData.auxiliaryUnitQty) {
            this.$set(findItem, 'auxiliaryUnitQty', changeNumData.auxiliaryUnitQty);
          }
          ...
          if (changeNumData.isHaveScIdFollowAmount) {
            ...
            // 重置下辅助单位数量
            if (changeNumData.auxiliaryUnitQty) {
              this.$set(findItem, 'auxiliaryUnitQty', multi(changeNumData.auxiliaryUnitQty, this.pkgMainData.lastQty));
            }
            // 重新设置下明细和套餐的比列
            this.$set(findItem, 'proportionNum', (changedNum * 1000) / (this.pkgMainData.lastQty * 1000));
          }
          // 判断改量是增加了还是减少了
          if (changedNum > this.activeItemData.count) {
            // 增加了 就调用加加 的方法 this.pkgItemChangeCount('pkgAdd',changeAddNum)
            const changeAddNum = (changedNum * 1000 - this.activeItemData.count * 1000) / 1000;
            this.$log.debug(`常规套餐改量：pkgAdd：${changeAddNum}`);
            // 这 执行了方法 如果是false 证明条件不符合不能关闭
            if (this.pkgItemChangeCount('pkgAdd', changeAddNum) != false) {
              this.$refs.PackageChangeNum.close();
            }
          } else if (changedNum == this.activeItemData.count) {
            // 相等的时候没有进行变化 直接关闭窗口
            this.$refs.PackageChangeNum.close();
          } else {
            // 比原先减少了就执行减的方法 pkgSubtract
            const changeSubNum = (this.activeItemData.count * 1000 - changedNum * 1000) / 1000;
            this.$log.debug(`常规套餐改量：pkgSubtract：${changeSubNum}`);
    ...
    // 改变右边的菜单的已点品项的数量 eventName 执行的方法, count 要改变的数值, item 要改变对应的菜品
    menuChangeCount(eventName, count, item) {
      // 改变右边菜单的数量
      this.pkgUnSelectedItems.forEach((menu) => {
        if (menu.itemId == (item ? item.itemId : this.activeItemData.itemId)) {
          switch (eventName) {
            case '+':
    ...
    /**
     * 套餐明细的修改数量的功做法
     * 包含删除,+,+ 改量,以及对已提交并且是联动修改
     * @controlBtnName {striing} 代表执行了加还是减还是删除的方法
     * @param {object}  findItem 找到当前要改变的品项进行单个的
     * @param {number} changeNum 要改变的数值
     * @param {object}  findMergeItem 找到当前要改变的itemId相同的所有品项的一个合并
     * @param {number}  index findItem 找到当前处于类的位置
     * @param {object} item 传入当前要改变的品项进行单个的
     * @param {object} mergeItem 传入当前要改变的itemId相同的所有品项的一个合并
     * @param {arr} pkgClassArr 当前操作的品项所在的类的数据组
     */
    // 加减 ,删除,改量的公共方法
    pkgItemChangeCount(controlBtnName, changeNum) {
      console.log('选中的品项：', this.findActivceItemData());
      this.$log.info(`常规套餐改量执行的方法：${controlBtnName}`);
      // 找到对应的数据 合并的和不合并的
      const { findItem } = this.findActivceItemData();
      if (!findItem) {
        // 提示
      ...
      if (controlBtnName == 'pkgSubtract' || controlBtnName == 'pkgDelete') {
        ...
        if (this.isRequiredFn(subCount)) {
          ...
          // 对应改变右侧的cout 数显示
          this.menuChangeCount('-', subCount, findItem);
          // 变化了就要重新赋值
        ...
        if (findItem.isConfirmWeigh || this.classTotal.isOrderRemainderNum >= addCount) {
          ...
          const itemLimitData = findMergeItem.itemLimitData || this.getItemLimitData(findMergeItem);
          if (itemLimitData) {
            // 限量所剩的和要加的做比较如果剩余小于要加的就提示
            if (sub(itemLimitData.quantity, findMergeItem.count) < addCount) {
              this.isShowMessage(this.$t('Biz.Order.LimitQuantityVal0', [itemLimitData.quantity]));
              return false;
            }
          }
          // 套餐明细下的品项限定判断
    ...
    PkgNormalTotal(pkgMainData) {
      this.$log.debug(`常规套餐总计数据：${pkgMainData}`);
      if (pkgMainData && JSON.stringify(pkgMainData) !== '{}') {
        // raisePriceType 属性是加价类型的判断 2:会员加价, 1:普通加价
        let additionalLastTotalPrice = pkgMainData.raisePriceType == 1 ? this.pkgNormalTotalRaisePrice.additionalTotalPrice : this.pkgNormalTotalRaisePrice.additionalMemberTotalPrice;
    ...
    addMenu(menu, index) {
      ...
      this.activeItemId = '';
      // 判断最大限量 true为没超过可以继续加单 加单的数量默认值为1
      if (this.classTotal.isOrderRemainderNum > 0) {
        const addPKgClass = this.classTotal.classData.activePkgDtList;
        this.addMenuData = menu;
        let menuCount = 0;
        let addMenuProportionNum = 1;
        // 这块 写活了  以前的count 默认就为1
        // 如果已入库的套餐并且是联动的那么加单的时候都是主套餐的数量 addMenuProportionNum 如果是已入库的需要算下和主套餐的比例
        if (this.scId && !this.pkgMainData.isNotFollowAmount) {
          menuCount = this.classTotal.isOrderRemainderNum >= this.pkgMainData.lastQty ? this.pkgMainData.lastQty : this.classTotal.isOrderRemainderNum;
          addMenuProportionNum = (menuCount * 1000) / (this.pkgMainData.lastQty * 1000);
        } else {
          // 加单数量默认为1
          menuCount = this.classTotal.isOrderRemainderNum >= 1 ? 1 : this.classTotal.isOrderRemainderNum;
        }
        // let menuCount = this.classTotal.isOrderRemainderNum >= num ? num : this.classTotal.isOrderRemainderNum
        ...
        const extendMenu = this.scId ? {
          ...
          itemName: menu.name,
          type: menuCount,
          proportionNum: addMenuProportionNum,
        } : {
          count: menuCount,
          itemCode: menu.code,
          itemName: menu.name,
        ...
        this.$log.debug(`常规套餐加单合并之后的数据：${itemData}`);
        // 改变菜单已点数量
    ...
    itemFilterFn(newPkgUnSelectedItem) {
      const itemFilterData = newPkgUnSelectedItem.filter((item) => {
        ...
        if (this.menuSearch.length > 0) {
          // quickSearch(this.menuSearch)
          let keyFilters = quickSearch((`${this.menuSearch}`).toLowerCase()); // 从这对数据进行处理
    ...
    pkgItemsSubmit() {
      // 套餐加单-限制数量（最大、最小值）
      if (!this.limitLastQty(this.pkgMainData.lastQty)) return;
      ...
      this.pkgNormalData.pkgClasses.forEach((_class) => {
        ...
        _class.activePkgDtList.map((_classItem) => {
          ...
          if (pointGroupId && _classItem.pointGroupId && _classItem.pointGroupId !== pointGroupId) {
            this.$message({
              type: 'error',
              message: this.$t('Biz.Order.LongMsg017', [_classItem.itemName]),
            });
            isSubmitTrue = false;
            return false;
          }
          // 判断默认的选择品项中是否存在了估清的品项
          if (_classItem.isSellout && !(this.scId && _classItem.scId)) {
            this.$message({
              type: 'error',
              message: this.$t('Biz.Order.LongMsg018', [_class.pkgClassName, _classItem.itemName]),
            });
            isSubmitTrue = false;
            return false;
          }
        ...
        if (_classNum < _class.choiceMinQty) {
          this.$message({
            type: 'error',
            message: this.$t('Biz.Order.LongMsg019', [_class.pkgClassName, _class.choiceMinQty]),
          });
          isSubmitTrue = false;
          return;
        }
        ...
        _class.mergePkgDtList.forEach((mergeClass) => {
          if (mergeClass.count > mergeClass.quantity && !(this.scId && _classItem.scId)) {
            this.$message({
              type: 'error',
              message: this.$t('Biz.Order.LongMsg020', [mergeClass.itemName]),
            });
            isSubmitTrue = false;
          }
        });
      ...
      if (isSubmitTrue) {
        if (this.scId) {
          this.submit(pkgSubmitData, this.pkgNormalData, this.skipKitchenPrint);
        } else {
          this.submit(pkgSubmitData, this.pkgNormalData);
        }
      }
    },
    // 套餐明细的数据整理
    ...
    creatItem(pkgItem) {
      // 判断是否是提交完的套餐修改
      let item = this.scId ? {
        itemId: pkgItem.realItemId, // 品项id
        realItemId: pkgItem.realItemId,
        itemSizeId: pkgItem.itemSizeId ? pkgItem.itemSizeId : -1, // 品项规格id
        code: pkgItem.itemCode, // 品项code
        name: pkgItem.itemName, // 品项名称
        unitId: pkgItem.itemUnitId, // 品项单位id
        unitName: pkgItem.itemUnitName, // 品项单位名称
        costPrice: pkgItem.costPrice, // 品项成本价格
        qty: pkgItem.count, // 已点的品项数量
        raisePrice: pkgItem.raisePrice, // 普通加价 价格
        isRaiseByQty: pkgItem.isRaiseByQty, // 是否按数量加价
        raiseMemberPrice: (pkgItem.raiseMemberPrice || pkgItem.raiseMemberPrice == 0) ? pkgItem.raiseMemberPrice : null, // 加价 价格
        methodList: pkgItem.methodList ? pkgItem.methodList : [], // 做法
        serve_way_id: (pkgItem.serveWay && pkgItem.serveWay.id) ? pkgItem.serveWay.id : (pkgItem.serveWay ? pkgItem.serveWay.split('_')[0] : 0), // 上菜方式
        type: pkgItem.type == 1 ? pkgItem.type : 0,
        scId: pkgItem.sc_id ? pkgItem.sc_id : null,
        raisePriceType: this.pkgMainData.raisePriceType == 2 && (pkgItem.raiseMemberPrice || pkgItem.raiseMemberPrice == 0) ? 2 : 1, // 加价的类型
      } : {
        id: pkgItem.realItemId, // 品项id
        sizeId: pkgItem.itemSizeId, // 品项规格id
        code: pkgItem.itemCode || pkgItem.code, // 品项code
        name: pkgItem.itemName || pkgItem.name, // 品项名称
        unitId: pkgItem.itemUnitId, // 品项单位id
        unitName: pkgItem.itemUnitName, // 品项单位名称
        costPrice: pkgItem.costPrice, // 品项成本价格
      ...
      // 增加辅助单位的key
      if (pkgItem.auxiliaryUnitId) {
        item = Object.assign({}, item, {
          auxiliaryUnitId: pkgItem.auxiliaryUnitId,
          auxiliaryUnitQty: pkgItem.auxiliaryUnitQty || 1,
          auxiliaryUnitName: pkgItem.auxiliaryUnitName,
        });
      }
      this.$set(item, 'apportionFlg', pkgItem.apportionFlg !== false); // 是否参与分摊 true 分摊 false 不分摊 null 分摊
      this.$log.debug(`常规套餐明细数据：${item}`);
      return item;
    },
    // 套餐的主要信息整理
    creatPkgMain(pkgMainData, pkgSubmitData) {
      ...
      if (this.scId) {
        pkgSubmitData.itemId = `${pkgMainData.id}_-1`;// 套餐的id
        pkgSubmitData.qty = pkgMainData.lastQty;// 套餐数
        pkgSubmitData.itemSizeId = pkgMainData.sizeId ? pkgMainData.sizeId : '-1';// 规格id
      } else {
      ...
      // pkgSubmitData.oldPrice = pkgMainData.oldPrice //老的价格
      pkgSubmitData.raisePrice = this.pkgNormalTotalRaisePrice.totalRaisePrice;// 套餐明细下普通加价的总和
      pkgSubmitData.raiseMemberPrice = this.pkgNormalTotalRaisePrice.totalRaiseMemberPrice;// 套餐明细下会员加价的总和
      ...
      // 促销价格
      // if (pkgMainData.promotePrice || pkgMainData.promotePrice) {
      ...
      this.$log.debug(`常规套餐信息数据：${pkgSubmitData}`);
    },
  },
  watch: {
    menuSearch() {
