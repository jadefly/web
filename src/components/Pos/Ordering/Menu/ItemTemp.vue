  ...
  >
    <div class="pos-dialog-content pos-ordering-itemtemp-wrapper">
      ...
      >
        ...
        <el-form-item :label="$t('Dict.Price01')">
          <el-input v-judge v-model="formItemTemp.stdPrice" v-num-keyboard:bottom id="tempItemStdPrice"></el-input>
        </el-form-item>
      </el-form>
      <div class="temp-small-type">
        <p>{{$t('Biz.Order.StaySubclass')}}</p>
        <ul :id="'smallClassTemp' + type">
          <li v-for="(item, index) in itemTempList" :class="{ 'active' : index === selectedIndex }" :key="item.itemId" @click="selectTempItem(item, index)">
            {{ item.smallClassName }}
          </li>
        </ul>
        <div class="small-type-footer">
          <Pager :showInfo="true" arrowDirection="up-down"
          :pagerId="'smallClassTemp' + type" :ref="'smallClassTemp' + type"></Pager>
        </div>
      </div>
      <div class="history-item-temp">
        <p>{{$t('Biz.Order.HistoryTempItems')}}</p>
        <ul :id="'historyTempItem' + type">
          <li v-for="(item, index) in filterHistoryTemp" :key="index"
          @click="selectHistoryTemp(item, index)" :class="{'active': index === selectHistoryIndex}">
        ...
        <div class="history-temp-footer">
          ...
          <Pager :showInfo="true" arrowDirection="up-down"
          :pagerId="'historyTempItem' + type" :ref="'historyTempItem' + type"></Pager>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    selectTempItem(item, index) {
      ...
      this.$refs[`historyTempItem${this.type}`].refresh();
    },
    // 选择该小类的历史临时品项
    ...
    onSubmit() {
      ...
      ajax('canyin.pos.bill.updateitemtemp', {
        ...
        if (data.success) {
          this.submit({ ...data.itData, smallClassId: this.itemTempList[this.selectedIndex].smallClassId });
    ...
    refreshPager() {
      setTimeout(() => {
        if (this.$refs[`historyTempItem${this.type}`]) {
          this.$refs[`historyTempItem${this.type}`].refresh();
        }
        if (this.$refs[`smallClassTemp${this.type}`]) {
          this.$refs[`smallClassTemp${this.type}`].refresh();
        }
      }, 400);
    },
  },
