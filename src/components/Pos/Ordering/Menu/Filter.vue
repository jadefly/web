<template>
  <div class="pos-order-menu-filter" :class="{'box_only_smallclass_oneLine': isSmallClassOneLine}">
    <div class="filter-item pos-order-tables-filter-item" v-if="!isOnlyShowSmallClass && !isShowCustomClass">
      <Paging
        :data="bigClasses"
    ...
    <div class="filter-item pos-order-tables-filter-item" :class="{'only-small': isOnlyShowSmallClass}" v-if="!isShowCustomClass">
      <Paging
        :data="smallClasses"
    ...
    <div class="filter-item pos-order-tables-filter-item only-small" v-if="isShowCustomClass">
      <Paging
        :data="customClasses"
...
export default {
  ...
  created() {
    ...
    this.$von('menuFilterPagerRefresh', () => {
      if (this.posItemCustomClasses && this.posItemCustomClasses.length) {
        this.$refs.customClassesList && this.$refs.customClassesList.refresh();
      } else {
        this.$refs.smallClassesList && this.$refs.smallClassesList.refresh();
        if (!this.isOnlyShowSmallClass) {
          this.$refs.bigClassesList && this.$refs.bigClassesList.refresh();
        }
      }
    });
  },
  ...
  computed: {
    ...
    smallClasses() {
      const finded = this.activeBigClass.id === ''
        ...
        : filter(this.posItemSmallClasses, { pid: this.activeBigClass.id });
        // 国际化：全部小类
      return concat({
        id: '',
