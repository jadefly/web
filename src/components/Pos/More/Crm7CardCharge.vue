<template>
  <Dialog
    name="PosDialog.Crm7CardCharge"
    :title="$t('Biz.Crm.Recharge')"
    ref="dialog"
    @close="onClose"
    @open="onOpen"
    ...
    height="669px"
    top="5vh">
    <div id="crm7CardChargeContainer" class="crm-container"></div>
  </Dialog>
</template>
...
export default {
  ...
  methods: {
    onOpen() {
      ...
      ajax('canyin.pos.settlement.pay.crmPayWay.crmCardCharge', {
        params: {
          crmVersion: 1,
        },
      }).then((res) => {
        $('#crm7CardChargeContainer').html(res.crmHtml);
      }).catch((code, msg) => {
        this.close();
        this.$message({
          message: msg,
