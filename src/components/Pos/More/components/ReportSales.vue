        ...
        <el-col :span="8">
          <div class="card" v-show="checkList.indexOf('4') >= 0">
            ...
            <table>
              ...
              <tr v-for="item in reportData.DS5">
                <td class="w40">{{item.name}}<template v-if="item.sizeName">({{item.sizeName}})</template></td>
                <td class="w20">{{item.count | keepTwo}}</td>
                <td class="w20">{{item.lastTotal | keepTwo}}</td>
                <td class="w20">{{item.reasonName}}</td>
              </tr>
              <tr v-if="reportData.DS7 && reportData.DS7[0]">
                <th class="w40">{{$t('Dict.Total')}}</th>
                <th class="w20">{{reportData.DS7[0].presentSumQty | keepTwo}}</th>
                <th class="w20">{{reportData.DS7[0].presentSumVal | keepTwo}}</th>
              </tr>
            </table>
          </div>
          <div class="card" v-show="checkList.indexOf('5') >= 0">
            ...
            <table>
              ...
              <tr v-if="reportData.DS7 && reportData.DS7[0]">
                <th class="w40">{{$t('Dict.Total')}}</th>
                <th class="w20">{{reportData.DS7[0].exitBillSumQty | keepTwo }}</th>
                <th class="w20">{{reportData.DS7[0].exitBillSumVal | keepTwo}}</th>
              </tr>
            </table>
          </div>
        </el-col>
...
export default {
  ...
  methods: {
    ...
    // 搜索
    search(operator) {
      let area = this.getIds(this.area, 'id');
      let shift = this.getIds(this.shift, 'id');
      let bigClassIds = this.getIds(this.bigClassIds, 'bigClassId');
      let smallClassIds = this.getIds(this.smallClassIds, 'smallClassId');
      let items = this.getIds(this.dishIds, 'itemId');
      ...
      ajax('canyin.pos.businessreport.b02salesreport', {
        ...
        data: {
          ...
          shift,
          state: this.state,
          tempItemShowType: this.tempItemShowType,
          bigClassIds,
          smallClassIds,
          items,
