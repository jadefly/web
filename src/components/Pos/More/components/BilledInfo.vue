          ...
          <div class="card">
            ...
            <table>
              <tr>
                ...
                <td class="table-content w65">{{tsData.posName || ""}}</td>
              </tr>
              <tr>
              ...
              <tr v-for="tax in taxList">
                <td class="table-title">{{tax.taxName || "服务费税"}}</td>
                <td class="table-content w75">{{tax.tax || ""}}</td>
              </tr>
        ...
        <div class="billed-info-item">
          ...
          <div class="card" v-else-if="billData.crmSettleData">
            ...
            <table>
              ...
              <tr v-if="billData.crmSettleData.balanceTicketMoney == 0 || billData.crmSettleData.balanceTicketMoney">
                <td class="table-title">{{$t('Biz.Order.LeftCoupon')}}</td>
...
export default {
  ...
  computed: {
    ...
    discountInfoStr() {
      ...
      if (discountInfo && discountInfo.length > 0) {
        for (let i = 0; i < discountInfo.length; i++) {
          ...
          if (discountInfo[i].dpName) { discountStr += `【${discountInfo[i].dpName}】<br>`; }
        }
    ...
    // 税
    taxList(){
      let { taxList } = this.tsData;
      // 服务费税数据调整
      if(taxList){
        let serverTax = taxList.find(item => item.taxType === 1);
        let isCombine = false;
        if(serverTax){
          ...
          if(isCombine){// 如果服务费税合并到其他税里，则taxList中去掉服务费税这一项
            taxList = taxList.filter(item => item.taxType !== 1);
          }
        }
      }
      return taxList ? taxList : [];
    }
  },
  methods: {
    upPage() {
