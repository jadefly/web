<template>
  <Dialog
    name="PosDialog.NSettleGroupAccounts"
    :title="payWayName"
    ref="dialog"
    @close="onClose"
    @open="onOpen"
    @submit="onSubmit"
    :hotkeys="NSettleGroupAccounts"
    :append-to-body="true"
    :showSubmit="true"
    :submit-text="$t('Biz.Settlement.ConfirmAccount')"
    top="7vh"
    width="980px"
    id="posDialogNSettleGroupAccounts"
  >
  ...
  <SmsVerificationModal
    ...
    @addPayWay="doSettle"
    @parentClose="close"></SmsVerificationModal>
  </Dialog>
</template>
...
import SmsVerificationModal from '@/components/Pos/Order/Bill/Pay/components/SmsVerificationModal';
  ...
  methods: {
      ...
      if (type === 'all') {
        ...
        this.groupList.forEach((item) => {
          if (!isEmpty(item.persons)) {
            allList = concat(allList, item.persons);
          }
        });
        this.payouts = allList;
        this.filterPayouts = allList;
    ...
    onSubmit(authCode = {}) {
      ...
      if (Number(this.beenAmountOfAccount) !== Number(this.amountToBePaid)) {
        this.$message.warning(this.$t('Biz.Settlement.AccountWarn03'));
        return false;
      }
      // 执行负结算
      this.doSettle();
    ...
    doSettle() {
      this.$emit(
        ...
        // 传参1-payData
        {
          ...
          accounts: this.addedAccountList,
        },
        // 传参2-authData写死空
        null,
        // 传参3-type请求结算类型
        'groupAccounts',
        // 传参4-结算完成的回调: 父组件完成执行回调传true,这里子组件才关闭页面
        (res) => {
          if (res) this.close();
        },
      );
