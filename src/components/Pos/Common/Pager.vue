<template>
  <div class="pos-pager">
    ...
    <button type="button" class="pager-btn"
      @click="next" :disabled="pagerState.current >= pagerState.total && scrollMode == 'data'">
...
export default {
  ...
  methods: {
    ...
    prev(ev) {
      ...
      if (ev) {
        this.$emit('setActiveIndex', this.pager, 'prev');
      }
    },
    next(ev) {
      ...
      if (ev) {
        this.$emit('setActiveIndex', this.pager, 'next');
      }
    },
  },
};
