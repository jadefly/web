...
import { ERR_OK, BACK_ORDER, INIT_PAGER } from '@/common/api/takeoutConfig.js';
...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      limit: 4,
      requestType: BACK_ORDER,
      currentPager: INIT_PAGER,
      multiQuery: '',
    };
  },
  created() {
    this.$von('refreshRefundOrder', () => {
      this.refreshData();
    });
  ...
  methods: {
      ...
      ajax('canyin.takeout.takeoutview.takeoutorderlist', {
        ...
        .then((res) => {
          if (res.data.code === ERR_OK) {
            ...
            this.$vemit('getBackTotal', this.total);
            if (callback) {
              callback();
            }
          } else {
    ...
    updateOrderList() {
      this.getOrderList(this.currentPager, () => {
        if (this.takeOutList.length === 0 && this.currentPager > 1) {
          this.getOrderList(this.currentPager - 1);
        }
      });
    },
  },
