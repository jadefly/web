<template>
  <div class="pos-takeout-window takeout-handling">
    <OrderFilter
    filterId="handlingFilter"
    ...
    <OrderList :takeOutList="takeOutList" :requestType="requestType" @getItem="getItem" ref="orderList" @cancelLast="cancelLast"></OrderList>
        ...
        <template v-if="orderItem && orderItem.viewSend">
          <el-tabs v-model="activeName" :stretch=true>
            <el-tab-pane label="配送跟踪" name="delivery">
              <DeliveryButtons :orderItem="orderItem" @updateOrderList="updateOrderList"
                               v-if="orderItem && (orderItem.canSendOrder || orderItem.canCancelAllSend)"></DeliveryButtons>
            ...
            <el-tab-pane label="订单详情" name="detail">
              <Buttons :orderItem="orderItem" @updateOrderList="updateOrderList" v-if="orderItem"></Buttons>
              <ItemList :orderItem="orderItem" :distributionItemName="distributionItemName"
                        :lunchboxItemName="lunchboxItemName" v-if="orderItem"></ItemList>
            </el-tab-pane>
          </el-tabs>
        </template>
        <!-- 发单后展示 -->
...
<script>
import { ERR_OK, HANDLING_ORDER, INIT_PAGER } from '@/common/api/takeoutConfig.js';
