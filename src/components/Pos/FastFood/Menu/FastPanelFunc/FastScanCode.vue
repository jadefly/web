    ...
    <div class="ScanCode">
      <div class="ScanCodeLeft">
        <div class="scancode-title">
          <img src="@/components/Pos/Order/Bill/Pay/images/Alipay2.png" alt="支付宝">
          <span class="name">{{$t('Dict.Alipay')}}</span>
          <img src="@/components/Pos/Order/Bill/Pay/images/WeChat2.png" alt="微信">
        ...
        <div v-if="!scanned">
          <form action="javascript:void(0)" v-keyboard:keyboard>
            ...
            <input type="text" ref="codeInput" v-model="code" :placeholder="$t('Biz.Settlement.Tip41')">
          </form>
        </div>
      ...
      <div class="ScanCodeRight">
        <div class="ScanCodeRight-tip"><i class="icon icon-tishi"></i>{{$t('Biz.Settlement.AlipayErrorMsg01')}}</div>
...
export default {
  ...
  methods: {
    ...
    enforceSettle() {
      ...
      this.$emit('enforceSettle', { payData: this.ScanCodeData, typeId: 507 });
    },
    manualSettle() {
      // typeId:6代表微信二维码;typeId:8代表支付宝付款码;typeId:18代表云闪付付款码
      this.$emit('manualSettle', { payData: this.ScanCodeData, typeId: 507 });
    },
