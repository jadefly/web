<template>
  <div class="pos-order-tables-search-wrap">
    <div
      ...
      v-switch-keyboard:right-right="keybordParams">
...
export default {
  ...
  props: {
    placeholderText: {
      type: String,
      default() {
        return this.$t('Biz.Order.SearchActivityPlan');
      },
    },
    duplexValue: {},
    searchId: {
  ...
  mounted() {
    ...
    // });
    // 挂载-搜索框及软键盘-功能重置方法
    this.$vonce('cancelSearchDisPlanFocusBus', () => {
      this.cancelSearchDisPlanFocus();
    });
    // 清空活动方案搜索input内容
    this.$vonce('resetSearchDisPlanText', () => {
      this.searchText = '';
    });
  },
  watch: {
    ...
    searchText(value, oldValue) {
      ...
      // 如果输入框为空分页按钮默认显示在第一页
      if (value === '') {
        // this.$vemit('selectDisPlanPageOrderOneBus');
...
.lang-enUS {
  .pos-order-tables-search-wrap .pos-order-tables-search > input {
    width: 200px;
  }
}
