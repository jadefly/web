    ...
    <div class="pos-dialog-content pos-team-bill-wrapper">
      <div class="left has-shadow team-general-info-wrapper">
        ...
        <div class="bill-content">
          <ul id="teamBillList">
            ...
            :key="index" @click="clickTeamBill(item, index)"
        ...
        <div class="bill-footer">
          ...
          pagerId="teamBillList" ref="teamBillList"></Pager>
      ...
      <div class="right team-detail-info-wrapper">
        ...
        <div v-if="selectedTab === 0" class="tab-content settlement-info">
          <div class="left">
            ...
            <dl>
              ...
              <dd><strong>{{$t('Dict.MinMakeUp')}}</strong><span>{{settleMsgTab.minFee}}</span></dd>
              <dd class="total"><strong>{{$t('Dict.Total')}}</strong><span>{{settleMsgTab.origTotal}}</span></dd>
            </dl>
            <!-- 国际化：优惠；折扣优惠；定额优惠；抹零；合计 -->
            <dl>
              <dt>{{$t('Dict.Offer')}}</dt>
              <dd><strong>{{$t('Biz.BizComm.Offer04')}}</strong><span>{{settleMsgTab.discMoney}}</span></dd>
              <dd><strong>{{$t('Biz.BizComm.Offer05')}}</strong><span>{{settleMsgTab.fixedDiscMoney}}</span></dd>
          ...
          <div class="right">
            ...
            <dl>
              ...
              <dd v-for="(payWay, index) in payWayInfo" :key="index">
                <strong>{{payWay.payWayName}}</strong><span>{{payWay.payMoney}}</span>
              </dd>
        ...
        <div v-if="selectedTab === 1" class="tab-content consume-detail">
          <!-- 国际化：品项；数量；价格；折扣；备注；小计 -->
          <div class="consume-detail-header">
            <span class="name">{{$t('Dict.Item')}}</span>
            <span class="quantity">{{$t('Dict.Btn.ChangeItemNum')}}</span>
          ...
          <div class="consume-detail-footer">
            <Pager :showInfo="true" arrowDirection="up-down"
            pagerId="consumeDetail" ref="consumeDetail"></Pager>
          </div>
        </div>
        <!-- 团队日志 -->
        <div v-if="selectedTab === 2" class="tab-content team-log">
          ...
          <div class="teamlog-footer">
            <Pager :showInfo="true" arrowDirection="up-down"
            pagerId="teamlogContent" ref="teamlogContent" scrollMode="simple"></Pager>
          </div>
        </div>
        <!-- 会员消费 -->
         ...
         <div class="left">
           <!-- 国际化：会员卡（券）明细；合计 -->
            <dl>
              ...
              <dd v-for="(crmPayWay, index) in crmPayWayInfo" :key="index">
                <strong>{{crmPayWay.crmTicketName}}</strong>
                <span class="card-num">{{crmPayWay.crmTicketCount}}</span>
                <span>{{crmPayWay.crmTicketMoney}}</span>
              </dd>
              <dd class="total"><strong>{{$t('Dict.Total')}}</strong><span>{{sumCrmPayWayMoney}}</span></dd>
            </dl>
          </div>
          <div class="right">
            <!-- 国际化：一卡通消费凭证；卡号；卡类型；会员姓名；会员电话号码；本次消费储值金额；本次消费赠送金额；消费电子券总额；本次抵现积分；
            本次产生积分；卡余额；剩余积分；剩余券总额 -->
            <dl v-if="crmSettleData">
              <dt>{{$t('Biz.Order.CardChargeCred')}}</dt>
              <dd><strong>{{$t('Biz.Order.DictCardNumber')}}</strong><span>{{crmSettleData.cardno}}</span></dd>
              <dd><strong>{{$t('Dict.CardType3')}}</strong><span>{{crmSettleData.cardkindtype}}</span></dd>
...
export default {
  ...
  methods: {
    ...
    clickTeamBill(item) {
      this.findData(item);
      if (this.$refs.consumeDetail) {
        this.$refs.consumeDetail.refresh();
      }
      if (this.$refs.teamlogContent) {
    ...
    changeShift() {
      ajax('canyin.pos.teamlog.shifttsinfo', {
        ...
        this.$message.error(this.$t('Biz.Order.NoAccountMsg'));
      });
    },
    // 初始化数据
    initData(resData) {
      ...
      // 结算信息tab
      this.settleMsgTab = (resData.tsList && resData.tsList.length > 0) ? resData.tsList[0] : null;
      ...
      // 定额优惠
      const fixedDisc = this.settleMsgTab.discTotal - this.settleMsgTab.wipeMoney
      - this.settleMsgTab.discMoney - this.settleMsgTab.presentMoney
      - this.settleMsgTab.promoteMoney - this.settleMsgTab.memberMoney;
      ...
      ajax('canyin.pos.teamloglist.view', {
        ...
        this.teamLogList.forEach((teamLog) => {
          this.$set(teamLog, 'info', `${teamLog.creator} ${teamLog.pos} ${teamLog.ip}`);
        });
      }).catch(() => {
        this.$message.error(this.$t('Biz.Order.LongMsg009'));
      });
    },
    onSearch(searchText) {
      const filterBills = this.teamBill.filter(bill => bill.tsCode.indexOf(searchText) !== -1);
      this.teamBill.forEach((bill) => {
        if (bill.tsCode.indexOf(searchText) === -1) {
    ...
    findData(item) {
      ...
      ajax('canyin.pos.teamlog.shifttsinfo', {
        ...
        ajax('canyin.pos.teamloglist.view', {
          ...
          this.$set(this.teamInfo, 'teamName', res.teamName);
          this.$set(this.teamInfo, 'openTime', res.openTime);
          this.$set(this.teamInfo, 'peopleQty', res.peopleQty);
          this.teamLogList = res.bills;
          this.teamLogList.forEach((teamLog) => {
            this.$set(teamLog, 'info', `${teamLog.creator} ${teamLog.pos} ${teamLog.ip}`);
          });
        }).catch(() => {
