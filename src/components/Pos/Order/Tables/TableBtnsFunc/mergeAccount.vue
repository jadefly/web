<template>
  <Dialog
    name="PosDialog.MergrAccount"
    :title="$t('Dict.Btn.MergeAccount')"
    ref="dialog"
    @close="onClose"
    @open="onOpen"
    @submit="onSubmit"
    ...
    top="10vh"
  >
    <div class="pos-dialog-content pos-merge-account-wrapper">
      <TableInfo class="merge-account-info" :infoData="infoData"></TableInfo>
      <TablesFilter
        ref="teamTableFilter"
        :areas="areas"
        :types="types"
...
export default {
  ...
  methods: {
    ...
    onOpen() {
      ...
      // 并账请求返回的所有信息
      this.mergrAccountData = this.resData;
      // 顶部显示信息
      this.infoData = {};
      ...
      // 当前客位菜品信息
      this.itemDatas = this.resData.serviceContents;
      // 初始化filter数据
      this.initFilterData();
      // 刷新过滤分页
      this.refreshPager('mergeAccount');
    },
    // 获取搜索框用的快捷键
    setKeySearchHotKey() {
    ...
    onSubmit(authData={}) {
      ...
      if (!this.itemDatas) {
        this.$message.error(this.$t('Biz.Order.LongMsg007'));
        return;
      }
      // 授权
      ...
      ajax('canyin.pos.mergebill.mergebill', {
        ...
        if (data.success) {
          ...
          this.close();
          this.$message.success(this.$t('Biz.Order.MergeAccountSuccess'));
        }
      }).catch((msgCode, msg) => {
