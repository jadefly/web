<template>
  <Dialog
      name="PosDialog.UnionPayQRCode"
      :title="$t('Dict.SweepCodePayment')"
      ref="dialog"
      @close="onClose"
      @open="onOpen"
      :hotkeys="UnionPayQRCode"
      :append-to-body="true"
      width="700px"
      height="650px"
    >
    <template slot="other-button">
      <button
       id="UnionPayQRCodeCloseBtn"
       type="button"
       class="btn btn-default"
       @click="CancellationOfPayment">{{$t('Dict.Close')}}(Esc)</button>
    </template>
    <div class="UnionPay">
      <div class="UnionPayLeft">
        <div>
          <img src="../images/UnionPay.png" alt="">
          <span class="name">{{$t('Biz.Settlement.UnionQR')}}</span>
        </div>
        <div class="line-through">
          <div class="price">{{qrCodeData.paymoney}}</div>
        </div>
        <div class="left-tip">{{$t('Biz.Settlement.Tip50')}}</div>
        <div class="UnionPayLeft-QRCode">
...
export default {
  ...
  created() {
    ...
    this.$von('closeSettleDialog', () => {
      this.$log.info('中餐银联二维码窗口被调用关闭');
      this.close();
    });
    this.$von('fastfoodCloseSettleDialog', () => {
      this.$log.info('快餐银联二维码窗口被调用关闭');
      this.close();
    });
  },
  methods: {
    onOpen() {
      $('#UnionPayQRCodeCloseBtn').siblings().css('display', 'none');
    ...
    enforceSettle(code = {}, dialog = 'UnionPayQRCode') {
      if (this.isTablePay) {
