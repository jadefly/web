...
import { COOK, SERVE, PACKAGE } from '@/store/order/orderConfig';
...
export default {
  ...
  computed: {
    ...
    groupCollection() {
      if (this.kitchenItemHisInfoList.length) {
        let attach = '';
        if (this.currentMode.currentState === COOK) {
        ...
        if (this.currentMode.currentState === PACKAGE) {
          attach = 'packageFinishTimeForTimeStamp';
        }
        ...
        return map(grouped, (items, key) => {
          ...
          if (this.currentMode.currentState === PACKAGE) {
            time = items[0].packageFinishTimeForTimeStamp;
          }
  ...
  methods: {
    ...
    onSubmit() {
      const cancelKcKscIdList = [];
      each(this.checked, (key) => {
        const finded = find(this.groupCollection, { $key: key });
        if (finded) {
          const { _kscIdArr, bsId } = finded;
          each(_kscIdArr, (kckscId) => {
            cancelKcKscIdList.push({ kckscId, bsId });
          });
        }
      });
      const posId = this.$route.params.posid;
      ajax(`canyin.kitchen.${this.currentMode.submitRevokeUrl}`, {
        contentType: 'json',
        data: { posId, cancelKcKscIdList },
      }).then(() => {
        this.close();
      });
    },
