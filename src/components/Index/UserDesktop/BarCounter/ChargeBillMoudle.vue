<template>
  <div class="bar-page">
      <SystemContent :title="$t('Biz.UserDesktop.RequisitionList')">
      <div class="bar-wrap">
        ...
        <div class="bar-content" ref="barContent">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                {{ scope.row.createTime | tDateFormat }}
              </template>
            </el-table-column>
            <el-table-column
                    prop="modifierName"
            ...
            >
              <template slot-scope="scope">
                <el-button
                  type="text"
                  size="mini"
                  @click="acquisition(scope.row.id)"
                >
    ...
    <Acquisition ref="acquisition"></Acquisition>
  </div>
</template>
...
<script>
  ...
  methods: {
    ...
    // 获取表格数据
    getTableData() {
      ajax('canyin.desktop.bizbmsreceive.list', {
    ...
    // 新建领用单
    acquisition(receiveId) {
      ajax('canyin.desktop.bizbmsreceive.getbmsreceivedetail', {
        contentType: 'URLEncoded',
        data: { receiveId },
      }).then((resData) => {
        if (resData.itemList && resData.itemList.length) {
          this.$refs.acquisition.open(resData, {
            submit: () => {
              this.reloadData();
            },
          });
    ...
    deleteItem(item) {
      ...
      ajax('canyin.desktop.bizbmsreceive.deleteitem', {
        contentType: 'URLEncoded',
