...
(function () {
    ...
    var webContextInfo = (function () {
        ...
        var browser = function () {
            if (cacheInfo.browser != null) return cacheInfo.browser;

            let br = {type: "Unknown", version: "0"};
            let explorer = window.navigator.userAgent.toLowerCase();
            //ie
            if (explorer.indexOf("msie") >= 0) {
                br.ver = explorer.match(/msie ([\d.]+)/)[1];
                br.type="IE"
            }
            //firefox
            else if (explorer.indexOf("firefox") >= 0) {
                br.ver = explorer.match(/firefox\/([\d.]+)/)[1];
                br.type="Firefox";
            }
            //Chrome
            else if (explorer.indexOf("chrome") >= 0) {
                br.ver = explorer.match(/chrome\/([\d.]+)/)[1];
                br.type= "Chrome";
            }
            //Opera
            else if (explorer.indexOf("opera") >= 0) {
                br.ver = explorer.match(/opera.([\d.]+)/)[1];
                br.type="Opera";
            }
            //Safari
            else if (explorer.indexOf("Safari") >= 0) {
                br.ver = explorer.match(/version\/([\d.]+)/)[1];
                br.type="Safari";
            }
         ...
         */
        let os = function () {
            if (cacheInfo.os != null) return cacheInfo.os;
            let ret = "Unknown";
            let sUA = navigator.userAgent.toLowerCase();
    ...
    var getPostData = function (msg, level) {
        ...
        if (msg instanceof Error) {
            body = extend(body, {name: msg.name, message: msg.message, stack: msg.stack, level: level});
        }
        else {
            body = extend(body, {name: "", message: msg + "", stack: "", level: level});
        }

        return body;
    }
    var post = function (msg, level) {
        var body = getPostData(msg, level);
        //依赖于jquery
        try {
    ...
    var config = function () {
        if (arguments.length == 0) return __config;
