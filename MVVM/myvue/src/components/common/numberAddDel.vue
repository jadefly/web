<template>
  <span class="numberAddDell">
    {{disabled_del}} {{ inputNum<=limitMin }}
...
import { decimal } from '@/assets/js/common/RegExpCheck';
import { add, multi, sub } from '@/assets/js/common/math';
export default {
  props: {
    inputNum: {
      // type: String,
    ...
    limit_Max(num, limitMax) {
      if (limitMax && num > limitMax) {
        ...
        if (this.showToast) {
          ...
          alert(`最大数量限制为${limitMax}`); // 最大数量限制为{0}
        }
        return { num, tag: false };
      };
      return { num, tag: true };
    ...
    limite(num) {
      ...
      if (this.limitMin && num < this.limitMin) {
        ...
        if (this.showToast) {
          ...
          alert(`最小数量限制为${this.limitMin}`); // 最小数量限制为{0}
        }
        return;
      }
      // 2.限量小于最大值时，先判断限量
