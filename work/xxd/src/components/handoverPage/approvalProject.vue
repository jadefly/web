  ...
  methods: {
    ...
    getProjectManger () {
      ...
      queryOrderFormByProjectId(param).then((item) => {
        ...
        this.listM.push({ label: '交付组', options: [] }, { label: '安服组', options: [] })
        this.projectMember = []
        this.memberList = []
        item.data.result.PROJECT_MEMBER.forEach((it) => {
          if (it.key == 0) {
            this.memberList.push({ departName: it.departName, id: it.id, realname: it.realname, username: it.username })
          } else {
    ...
    submitForm () {
      let form = {
        ...
        projectMember: '667,6321b7334e34c1872e1de994ee7bb0be',
        receiptID: this.receiptID,
